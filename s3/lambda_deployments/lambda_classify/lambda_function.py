...
def lambda_handler(event, context):
    ...
    for i, bill in enumerate(to_classify):
        ...
        # Incremental save every 100 bills — prevents losing progress on timeout
        if (i + 1) % 100 == 0:
            save_json_to_s3(CLASSIFIED_KEY, classified)
            if new_matches:
                batch_count = len(new_matches)
                total_matched += batch_count
                save_matches_csv(new_matches)
                new_matches = []  # reset — each batch gets its own CSV
                print(f"  Checkpoint {i+1}/{len(to_classify)}: "
                      f"saved tracker + {batch_count} matches")
            else:
                print(f"  Checkpoint {i+1}/{len(to_classify)}: "
                      f"saved tracker (no new matches in this batch)")

    # 6. Write remaining matches (since last checkpoint)
    matches_csv_key = None
    if new_matches:
        total_matched += len(new_matches)
        matches_csv_key = save_matches_csv(new_matches)
        print(
            f"  Final batch: {len(new_matches)} matches → s3://{BUCKET}/{matches_csv_key}")

    # 7. Save classification tracker (final)
    save_json_to_s3(CLASSIFIED_KEY, classified)

    print(f"\nResults:")
    print(f"  Classified: {len(to_classify)}")
    print(f"  LGBTQ+ relevant: {total_matched}")
